# -*- coding: utf-8 -*-
"""Sadece yeni sehir sayfalarini uretir (diger sayfalara dokunmaz)."""
import io
import os
import sys
...
for slug in ("sanliurfa", "kahramanmaras", "kastamonu"):
    info = sp.CITIES[slug]
    html = sp.render_city(slug, info, keys)
    ...
    for folder in ("rootsite", "docs"):
        ...
        if url not in sm:
            sm = sm.replace(
                "</urlset>",
                "  <url><loc>{0}</loc><lastmod>2026-09-13</lastmod></url>\n</urlset>".format(url),
            )
